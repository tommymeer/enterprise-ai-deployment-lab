...
from .extraction_evaluation import (evaluate_extraction_case, get_extraction_eval_cases,
    get_semantic_robustness_eval_cases, run_scripted_semantic_robustness_eval,
    scripted_response)
...
def eval_evidence():
    ...
    semantic_cases=get_semantic_robustness_eval_cases()
    ...
    failure_runs=tuple(run_failure_regression_case(case) for case in get_failure_regression_cases())
    failure_evaluations=tuple(evaluate_failure_regression_case(run) for run in failure_runs)
    duplicate_run=run_failure_regression_case(get_failure_regression_case("duplicate-suppresses-injection"))
    duplicate=evaluate_failure_regression_case(duplicate_run)
    extraction_fields=tuple(comparison for result in extraction_results for comparison in result.field_comparisons)
    semantic_failures=tuple(result for result in semantic_results if result.semantic_match is not True)
    invented_message=scenarios["invented-order"].customer_message
    customer_message_identifier=next(token.rstrip(".,!?") for token in invented_message.split() if token.startswith("ORD-"))
    model_output_identifier=json.loads(scenarios["invented-order"].response.response_text or "{}").get("order_identifier")
    duplicate_events={event.event_type for event in duplicate_run.workflow_result.trace_events}
    duplicate_operation=duplicate_run.workflow_result.execution_operation
    forbidden_downstream_events_observed=sum(
        event_type in {event.event_type for event in run.workflow_result.trace_events}
        for run in failure_runs for event_type in run.regression_case.expected_forbidden_trace_events
    )
    authority_block=next(event for event in over_limit.trace_events if event.event_type=="execution_authority_blocked")
    authority_input=authority_block.tool_arguments
    trajectory_failures=check_trajectory(reordered,altered)
    coverage=[
        {"group":"Extraction behavior","title":"Extraction correctness","detail":"Checks the nine-field extraction contract against 10 synthetic messages.","why":"Incorrect extraction can misroute the case or introduce bad structured context before trusted retailer evidence is retrieved.","method":"Deterministic fixture replay + deterministic grader","passed":all(result.semantic_match for result in extraction_results),"evidence":{"cases":len(extraction_results),"cases_passed":sum(result.semantic_match is True for result in extraction_results),"fields_per_case":len(extraction_results[0].field_comparisons),"expected_fields_matched":sum(comparison.matched for comparison in extraction_fields),"expected_fields_total":len(extraction_fields)}},
        {"group":"Extraction behavior","title":"Semantic robustness","detail":"Keeps structured facts stable across paraphrase, fact order, irrelevant detail, and verbosity changes.","why":"Meaning-preserving wording changes should not cause materially different structured extraction.","method":"Deterministic fixture replay + deterministic grader","passed":all(result.semantic_match for result in semantic_results),"evidence":{"source_cases":len({case.source_case_id for case in semantic_cases}),"variants":len(semantic_results),"variants_passed":sum(result.semantic_match is True for result in semantic_results),"failures":len(semantic_failures),"transformations_per_source":len(semantic_results)//len({case.source_case_id for case in semantic_cases})}},
        {"group":"Extraction behavior","title":"Grounding / hallucination control","detail":"Rejects an order identifier that is absent from the customer message before workflow routing.","why":"An invented order identifier could cause the workflow to retrieve or act on the wrong customer record.","method":"Deterministic fixture replay + deterministic validator","passed":guard.status.value=="invalid_model_output","evidence":{"fixture":"invented-order","customer_message_identifier":customer_message_identifier,"model_output_identifier":model_output_identifier,"model_output_identifier_grounded":bool(model_output_identifier and model_output_identifier in invented_message),"validator_result":"rejected" if guard.status.value=="invalid_model_output" else guard.status.value,"workflow_entered":False}},
        {"group":"Workflow correctness","title":"Trajectory correctness","detail":"Checks event order independently of final outcome, so a correct result cannot conceal an unsafe sequence.","why":"A correct final result is not sufficient if the system executes a consequential action before the required decision or authorization step.","method":"Deterministic fixture replay + deterministic trace grader","passed":not check_outcome(reordered,expected) and bool(trajectory_failures),"evidence":{"fixture":"correct_outcome_execution_before_disposition","expected_violation":"execution_before_disposition","outcome_evaluator_failures":len(check_outcome(reordered,expected)),"evaluator_detected_violation":bool(trajectory_failures),"trajectory_failures":list(trajectory_failures),"result":"PASS"}},
        {"group":"Safety / authorization","title":"Authorization safety","detail":"Blocks an over-limit refund before execution and routes it to authorized human review.","why":"The system may select the correct business resolution but still lack permission to execute it autonomously.","method":"Deterministic fixture replay + deterministic workflow / trajectory grader","passed":not check_outcome(over_limit,over_limit_expected) and not check_trajectory(over_limit,over_limit_events),"evidence":{"fixture":"over_limit_refund","proposed_amount":authority_input["refund_amount_minor"]//100,"autonomous_limit":authority_input["autonomous_limit_minor"]//100,"currency":authority_input["currency"],"authorization":"blocked","execution_started":any(event.event_type=="execution_started" for event in over_limit.trace_events),"final_route":over_limit.final_case_status.value}},
        {"group":"Reliability / recovery","title":"Failure handling","detail":"Checks controlled dependency and execution failures for the intended safe route and forbidden downstream events.","why":"Dependency or execution failures must stop or escalate safely instead of producing false success.","method":"Deterministic fixture replay + deterministic regression grader","passed":all(evaluation.passed for evaluation in failure_evaluations),"evidence":{"regression_cases":len(failure_evaluations),"controlled_failure_cases":sum(run.regression_case.expected_failure_stage is not None for run in failure_runs),"cases_passed":sum(evaluation.passed for evaluation in failure_evaluations),"safe_routes_preserved":all(run.workflow_result.final_case_status==run.regression_case.expected_case_status for run in failure_runs),"forbidden_downstream_events_observed":forbidden_downstream_events_observed,"result":"PASS" if all(evaluation.passed for evaluation in failure_evaluations) else "FAIL"}},
        {"group":"Safety / authorization","title":"Idempotency","detail":"Verifies a shared execution registry suppresses a duplicate consequential call.","why":"Retries or duplicate requests must not create duplicate consequential actions.","method":"Deterministic fixture replay + deterministic regression grader","passed":duplicate.passed,"evidence":{"fixture":"duplicate-suppresses-injection","idempotency_key":duplicate_operation.idempotency_key if duplicate_operation else None,"execution_adapter_calls":duplicate_run.adapter_invocations,"duplicate_suppressed":"duplicate_execution_suppressed" in duplicate_events,"final_case_status":duplicate_run.workflow_result.final_case_status.value,"result":"PASS" if duplicate.passed else "FAIL"}},
    ]
    item_details = {
