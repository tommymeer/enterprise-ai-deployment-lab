"""Manually authorized, bounded live evaluation of the curated extraction cases."""
...
AUTHORIZED_ESTIMATED_SPEND_LIMIT_USD = 0.15
...
def _cases() -> tuple[ExtractionEvalCase, ...]:
    cases = get_extraction_eval_cases()
    if len(cases) != MAX_ATTEMPTED_CALLS or len({case.case_id for case in cases}) != 10:
        raise RuntimeError("live extraction evaluation requires exactly ten unique cases")
    return cases
