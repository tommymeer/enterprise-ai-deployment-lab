...
class DemoViewTests(unittest.TestCase):
    ...
    def test_eval_examples_are_existing_fixtures(self):
        ...
        technical={item["title"]:item["evidence"] for item in evidence["coverage"]}
        self.assertEqual(technical["Extraction correctness"],{"cases":10,"cases_passed":10,"fields_per_case":9,"expected_fields_matched":90,"expected_fields_total":90})
        self.assertEqual(technical["Semantic robustness"],{"source_cases":5,"variants":20,"variants_passed":20,"failures":0,"transformations_per_source":4})
        self.assertEqual(technical["Grounding / hallucination control"],{"fixture":"invented-order","customer_message_identifier":"ORD-9009","model_output_identifier":"ORD-9999","model_output_identifier_grounded":False,"validator_result":"rejected","workflow_entered":False})
        self.assertEqual(technical["Trajectory correctness"]["trajectory_failures"],["disposition must occur before execution_started"])
        self.assertEqual(technical["Authorization safety"],{"fixture":"over_limit_refund","proposed_amount":150,"autonomous_limit":100,"currency":"USD","authorization":"blocked","execution_started":False,"final_route":"human_review"})
        self.assertEqual(technical["Failure handling"],{"regression_cases":10,"controlled_failure_cases":9,"cases_passed":10,"safe_routes_preserved":True,"forbidden_downstream_events_observed":0,"result":"PASS"})
        self.assertEqual(technical["Idempotency"],{"fixture":"duplicate-suppresses-injection","idempotency_key":"support-case:synthetic-case-refund-success:approve_refund","execution_adapter_calls":1,"duplicate_suppressed":True,"final_case_status":"closed","result":"PASS"})
